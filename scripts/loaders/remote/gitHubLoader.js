...
var GitHubLoader = function (_ExpandedLoader) {
  ...
  function GitHubLoader(projectType, slug, initialPath) {
    ...
    _this.loader = projectType === "code" ? new ProjectLoader() : new ProjectBlocksLoader();

    _this.CLIENT_ID = "086085200026d5c54c19";
    _this.CLIENT_SECRET = "f40981be76b00e35d4437d71184f42a70d08f3a6";
  ...
  _createClass(GitHubLoader, [{
    ...
    value: function load(getContext, callback) {
      ...
      return this._loadDir(this.initialPath).then(function (files) {
        ...
        _this2.loader.readRaw(getContext(), _this2._createZip(files), callback);
      });
    }
  }, {
    key: "scanDir",
    value: function scanDir() {
      ...
      return $.get("https://api.github.com/repos/" + username + "/" + repoName + "/contents/" + path + "?client_id=" + this.CLIENT_ID + "&client_secret=" + this.CLIENT_SECRET);
    }
  }, {
    key: "_loadDir",
    value: function _loadDir() {
      var _this3 = this;
    ...
    value: function _loadEntry(entry, path) {
      return entry.type === "dir" ? this._loadDir(path + "/" + entry.name) : this._downloadFile(entry.download_url, entry.path);
    }
  }, {
    key: "_downloadFile",
